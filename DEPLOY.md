...
| what | value |
|---|---|
| app | `apps/ask` (`@atpost/ask`), basePath `/ask`, dev port 3013, e2e port 3023 |
| image | ECR `atpost/web-ask` (`ask` is in the `build-push.yml` matrix) |
| ArgoCD values | `deploy/web/ask/values-<env>.yaml` in modernsmapp (does not exist yet; copy an existing zone's, e.g. `deploy/web/miniapps/`) |
| ALB path | `/ask` and `/ask/*` on the shared `atpost-web-<env>` group, ordered before the shell's `/` catch-all |
| shell env | `ASK_ZONE_URL` = the ask Service URL (local default `http://localhost:3013`) |
...
everyone not in `QA_PILOT_USER_IDS` (and every signed-out visitor) sees "Ask
isn't available yet". See `docs/ask-web.md`.

...
describe("VoteView", () => {
  ...
  it("signed out, the arrows are sign-in links", () => {
    const href = askSignInHref("/questions/abc")
...
describe("states", () => {
  it("the gate screen says Ask isn't available", () => {
...
describe("askSignInHref", () => {
  it("returns to the Ask page the reader was on", () => {
    expect(askSignInHref("/")).toBe("/login?redirect=%2Fask")
    expect(askSignInHref("/questions/abc")).toBe("/login?redirect=%2Fask%2Fquestions%2Fabc")
    expect(askSignInHref("new")).toBe("/login?redirect=%2Fask%2Fnew")
  })
})

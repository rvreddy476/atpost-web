...
/** `/kwit/topics` — the topic directory. Topics are moderator-created; there is no create UI. */
...
function TopicFollow({ topic, signedIn }: { topic: AskTopic; signedIn: boolean }) {
  ...
  if (!signedIn) {
    return (
      <a href={kwitSignInHref(`/topics/${topic.id}`)} className={`${PILL_ACTION} py-1`}>
        {COPY.follow}
      </a>
    )
  }
...
/** `/kwit/topics/[id]` — a topic's header and its questions. */

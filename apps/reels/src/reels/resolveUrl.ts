 ...
 *
 * The same function apps/social hands its player, for the same reason, and it
 * is worth stating again because getting it wrong produces a symptom that
 * points nowhere: the feed hands out `hls_url` / `playback_url` as
 * `/v1/media/{id}/hls/master.m3u8`, and the master playlist's children are
 * origin-absolute paths of the same shape. Both resolve against the ORIGIN, so
 * under `basePath: "/reels"` they land on `/v1/...` — a path this zone does
 * not serve. A reel then fetches its master playlist, reports no error, and
 * shows a black frame for ever.
 *
 ...
 * sees one it is `http://localhost:3011/v1/media/...` — a full url that starts

...
/**
 * Smoke tests for the Ask zone (apps/ask, basePath /ask, e2e port 3023)
 ...
 * here; these specs assume the Ask screens read qa-service from the client,
 * like the shop does.
 */

const BASE = 'http://127.0.0.1:3023/ask'
...
async function mockApi(page: Page, qa: QaHandler) {
  await page.route('**/v1/**', async (route) => {
    ...
    const path = url.pathname.replace(/^\/ask/, '')
...
test('signed out, /ask renders the trending feed and topics', async ({ page }) => {
  const asked: string[] = []
...
test('the gateway dormant gate shows "Ask isn\'t available yet"', async ({ page }) => {
  ...
  await expect(page.getByText(/isn.t available/i).first()).toBeVisible()
